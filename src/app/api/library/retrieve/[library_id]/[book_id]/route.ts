...
export async function GET(request: Request, { params }: { params: { library_id: string, book_id: string } }) {
    try {
        ...
        return NextResponse.json({ error: 'Invalid request', debug: error }, { status: 400 });
    }
}